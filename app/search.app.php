...
//            echo ' come the initial process' . 'the count is ' . count($_GET);
         ...
         {
             /* $goods_list_keyword = $goods_mod->get_list2(array(
                'conditions' => $conditions." AND ".db_create_in(array('3785661'),'g.goods_id'),
                'order' => isset($_GET['order']) && isset($orders[trim(str_replace('asc', '', str_replace('desc', '', $_GET['order'])))]) ? $_GET['order'] : 'sort_order asc', // tyioocom
             ...
             $goods_list_keyword = $goods_mod->get_list(array(
                 'conditions' => "g.if_show = 1 AND g.closed = 0 AND g.default_spec > 0 AND s.state = 1 AND ".db_create_in(array('4261069'),'g.goods_id'),
